...
class BlockDocumentManager {
    constructor(masterYdoc) {
        this.blockIdMap = new Map(); // ProseMirror node -> block_id
        this.childDocMap = new Map(); // block_id -> childYdoc (本地缓存)
        this.masterYdoc = masterYdoc;
        this.masterIndex = masterYdoc.getMap('index');
        this.masterData = masterYdoc.getMap('data'); // 存储 GUID
        // 监听 masterData 的变化，同步子文档（从 GUID 加载）
        this.masterData.observe((event) => {
            event.keysChanged.forEach((blockId) => {
                if (this.masterData.has(blockId)) {
                    ...
                    if (childDocGuid && !this.childDocMap.has(blockId)) {
                        ...
                        if (foundChildDoc) {
                            ...
                            if (typeof foundChildDoc.load === 'function') {
                                try {
                                    foundChildDoc.load();
                                }
                                catch (e) {
        ...
        // 监听子文档的加载
        this.masterYdoc.on('subdocs', (subdocs) => {
            subdocs.added.forEach((childYdoc) => {
                console.log(`📥 子文档已加载: ${childYdoc.guid}`);
            });
            subdocs.removed.forEach((childYdoc) => {
    ...
    // 为第一层节点创建或获取子文档
    getOrCreateChildDoc(node, fractionalIndex) {
        ...
        if (this.masterData.has(blockId)) {
            ...
            if (!childDocGuid) {
                ...
                if (typeof childYdoc.load === 'function') {
                    try {
                        childYdoc.load();
                    }
                    catch (e) {
            ...
            if (this.childDocMap.has(blockId)) {
                ...
                const guidToFind = childDocGuid; // 类型断言，确保不是 undefined
    ...
    getChildDefaultFragment(blockId) {
        const childYdoc = this.childDocMap.get(blockId);
        if (!childYdoc) {
            return null;
        }
...
export const BlockDocumentExtension = Extension.create({
                ...
                view(editorView) {
                    ...
                    const updateBlockDocs = (view) => {
                        ...
                        if (firstLevelNodes.length > 0) {
                            ...
                            // 列出所有 subdocs
                            const subdocGuids = [];
                            masterYdoc.subdocs.forEach((doc) => {
    ...
    onCreate() {
        ...
        if (collaborationExt) {
            // 尝试多种方式获取 document
            masterYdoc = collaborationExt.options?.document ||
                collaborationExt.storage?.document ||
                collaborationExt.props?.document;
        }
        // 如果还是找不到，尝试从 editor 实例中获取
        if (!masterYdoc && editor.collaboration?.document) {
            masterYdoc = editor.collaboration.document;
        }
        if (masterYdoc) {
    ...
    onDestroy() {
        if (this.storage.blockManager) {
            ...
            allChildDocs.forEach((_childYdoc, blockId) => {
